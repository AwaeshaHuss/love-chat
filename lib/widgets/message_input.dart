...
//   _typingTimer?.cancel();
//   _typingTimer = Timer(const Duration(milliseconds: 625), () => _updateTypingStatus(false));
// }
...
class _MessageInputState extends State<MessageInput> {
  ...
  void _sendMessage({String? mediaUrl, bool isVideo = false}) {
    if (_controller.text.trim().isEmpty && mediaUrl == null) return;
    ...
    final messageData = {
      ...
      'imageUrl': isVideo ? '' : mediaUrl ?? '',
      'videoUrl': isVideo ? mediaUrl ?? '' : '',
      "participants": [widget.currentUserId, widget.recipientUserId],
    };
  ...
  String _getChatId() {
    final users = [widget.currentUserId, widget.recipientUserId];
    users.sort();
    return users.join('_');
  }
